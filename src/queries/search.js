...
export default async function typesenseSearch(
  ...
  connectionArgs.typesenseSearchParameters.q =
    connectionArgs.typesenseSearchParameters.q.join(",");
  connectionArgs.typesenseSearchParameters.query_by =
    connectionArgs.typesenseSearchParameters.query_by.join(",");
  try {
    if (
      connectionArgs.typesenseSearchParameters.query_by.indexOf("tagIds") > -1
    ) {
      const qstring = connectionArgs.typesenseSearchParameters.q.split(",");
      const qarray = qstring.map(
        (tagId) => Buffer.from(tagId, "base64").toString().split(":")[1]
      );
      if (qarray.length > 0) {
    ...
    if (
      connectionArgs.typesenseSearchParameters.filter_by.indexOf("tagIds") > -1
    ) {
      const qstring =
        connectionArgs.typesenseSearchParameters.filter_by.split("&&");
      const qarray = qstring.map((tagId) => decodeOpaqueId(tagId.split(":")));
      if (qarray.length > 0) {
        connectionArgs.typesenseSearchParameters.filter_by =
          qarray.join(" && ");
      }
    }
  } catch (error) {}
  ...
  const perPage = Math.min(
    connectionArgs?.typesenseSearchParameters?.first || 20,
    100
  );
  ...
  const searchData = await searchtypeSense(
    connectionArgs.typesenseSearchParameters
  );
  ...
  const searchDataWithCatalogProducts = searchData.hits.map((hit) => {
    ...
    catalogProduct.pricing.PLN.compareAtPrice = fixComparePrice(
      catalogProduct.pricing.PLN
    );

    // Not sure but shuold be rewriteed
    catalogProduct.pricing = [
      { code: "PLN", currency: { code: "PLN" }, ...catalogProduct.pricing.PLN },
    ];
    catalogProduct.variants = catalogProduct.variants.map((va) => {
      va.pricing.PLN.compareAtPrice = fixComparePrice(va.pricing.PLN);
      va.pricing = [
        { code: "PLN", currency: { code: "PLN" }, ...va.pricing.PLN },
      ];
  ...
  const searchResponse = await xformArrayToConnection(
    {},
    searchDataWithCatalogProducts
  );
